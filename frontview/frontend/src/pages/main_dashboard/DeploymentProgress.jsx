...
export default function DeploymentProgress() {
  ...
  const connectSocket = (id) => {
    ...
    socket.on('deployment:status', (data) => {
      console.log("ðŸ“¡ Deployment Status Update:", data);
    ...
    socket.on('deployment:screenshot', (data) => {
      console.log("ðŸ“¸ Deployment Screenshot Update:", data);
          ...
          <div className="max-w-6xl mx-auto w-full flex flex-col h-full">
            ...
            <div className="flex items-center justify-between mb-3 pb-3 border-b border-white/[0.04] shrink-0">
              ...
              <div className="flex items-center gap-2">
                {/* Redeploy Button â€” One click to trigger latest */}
                <button
                  ...
                  <div className="text-left">
                    ...
                    {latestCommit && !isDeploying && (
                      <div className="text-[7px] text-[#3f3f46] font-mono truncate max-w-[120px]">
                        {latestCommit.sha?.slice(0,7)} Â· {latestCommit.commit?.message?.slice(0,18)}
                      </div>
                    )}
                  </div>
                </button>
...
{isLive && (
  ...
  {isLive ? 'NOMINAL' : status === 'failed' ? 'FAULT' : status === 'stopped' ? 'STOPPED' : 'SYNCING'}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 flex-1 min-h-0">
              {/* Left Panel â€” Site Preview Card */}
                    ...
                    {latestCommit && (
                      <div className="flex items-center gap-2 mt-2 pt-2 border-t border-white/[0.04]">
                        ...
                        <div className="min-w-0">
                          <p className="text-[8px] font-mono text-[#3f3f46] truncate">
                            <span className="text-[#22c55e]">{latestCommit.sha?.slice(0,7)}</span> Â· {latestCommit.commit?.message?.slice(0,28)}
                          </p>
                        </div>
                      </div>
                    )}
