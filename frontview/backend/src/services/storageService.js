...
const uploadFile = async (remotePath, body, contentType = 'application/octet-stream') => {
    try {
        ...
        console.log(`✅ File uploaded to Storj S3: ${remotePath}`);
        return remotePath;
    } catch (error) {
        console.error(`❌ Storj S3 Upload Failed: ${error.message}`);
        throw error;
    }
};
...
const uploadDirectory = async (localPath, remotePrefix) => {
    try {
        ...
        console.log(`🚀 Parallel upload started: ${files.length} files found.`);
        ...
        console.log(`✅ Directory upload completed: ${remotePrefix}`);
